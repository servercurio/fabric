 ...
 *
 * <p>
 * This class utilizes the Java2D "best practices" for image manipulation, ensuring that all operations (even most
 * user-provided {@link BufferedImageOp} s) are hardware accelerated if provided by the platform and host-VM.
 *
 * <p>
 * <h3>Image Quality</h3>
 * This class implements a few different methods for scaling an image, providing either the best-looking result, the
 * fastest result or a balanced result between the two depending on the scaling hint provided (see {@link Method}).
 *
 * <p>
 ...
 * <p>
 * The results generated by imgscalr using this method, as compared to a single {@link
 * RenderingHints#VALUE_INTERPOLATION_BICUBIC} scale operation look much better, especially when using the {@link
 * Method#ULTRA_QUALITY} method.
 *
 * <p>
 * Only when scaling using the {@link Method#AUTOMATIC} method will this class look at the size of the image before
 * selecting an approach to scaling the image. If {@link Method#QUALITY} is specified, the best-looking algorithm
...
public class Scalr {
    ...
    /**
     * Flag used to indicate if debugging output has been enabled by setting the "<code>imgscalr.debug</code>" system
     * property to <code>true</code>. This value will be <code>false</code> if the "<code>imgscalr.debug</code>" system
     ...
     * new property value for {@link #DEBUG_PROPERTY_NAME} before this class is loaded.
     *
     * <p>
     ...
     * </code> or by calling {@link System#setProperty(String, String)} to set a
     * new property value for {@link #LOG_PREFIX_PROPERTY_NAME} before this class is loaded.
     *
     * <p>
     ...
     *
     * <p>
     * A common request by users of the library was that they wished to "soften" resulting images when scaling them down
     * drastically. After quite a bit of A/B testing, the kernel used by this Op was selected as the closest match for
     * the target which was the softer results from the deprecated {@link AreaAveragingScaleFilter} (which is used
     * internally by the deprecated {@link Image#getScaledInstance(int, int, int)} method in the JDK that imgscalr is
     * meant to replace).
     *
     * <p>
    ...
    /**
     * Threshold (in pixels) at which point the scaling operation using the {@link Method#AUTOMATIC} method will decide
     * if a {@link Method#BALANCED} method will be used (if smaller than or equal to threshold) or a {@link
     * Method#SPEED} method will be used (if larger than threshold).
     *
     * <p>
     * The bigger the image is being scaled to, the less noticeable degradations in the image becomes and the faster
     * algorithms can be selected.
     *
     * <p>
     * The value of this threshold (1600) was chosen after visual, by-hand, A/B testing between different types of
     * images scaled with this library; both photographs and screenshots. It was determined that images below this size
     * need to use a {@link Method#BALANCED} scale method to look decent in most all cases while using the faster {@link
     * Method#SPEED} method for images bigger than this threshold showed no noticeable degradation over a
     * <code>BALANCED</code> scale.
     */
    public static final int THRESHOLD_BALANCED_SPEED = 1600;
    /**
     * Threshold (in pixels) at which point the scaling operation using the {@link Method#AUTOMATIC} method will decide
     * if a {@link Method#QUALITY} method will be used (if smaller than or equal to threshold) or a {@link
     ...
     *
     * <p>
     * The value of this threshold (800) was chosen after visual, by-hand, A/B testing between different types of images
     * scaled with this library; both photographs and screenshots. It was determined that images below this size need to
     * use a {@link Method#QUALITY} scale method to look decent in most all cases while using the faster {@link
     * Method#BALANCED} method for images bigger than this threshold showed no noticeable degradation over a
     ...
     * href="http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=4965606">a decade-old JDK bug</a> that can cause a
     * {@link RasterFormatException} when applying a perfectly valid {@link BufferedImageOp}s to images.
     *
     * <p>
     * <strong>Feature</strong>: This implementation also works around
     * {@link BufferedImageOp}s failing to apply and throwing {@link ImagingOpException}s when run against a
     * <code>src</code> image type that is poorly supported. Unfortunately using {@link ImageIO} and standard Java
     * methods to load images provides no consistency in getting images in well-supported formats. This method
     * automatically accounts and corrects for all those problems (if necessary).
     *
     * <p>
     * It is recommended you always use this method to apply any {@link BufferedImageOp}s instead of relying on directly
     * using the {@link BufferedImageOp#filter(BufferedImage, BufferedImage)} method.
     *
     * <p>
     * <strong>Performance</strong>: Not all {@link BufferedImageOp}s are
     * hardware accelerated operations, but many of the most popular (like {@link ConvolveOp}) are. For more information
     * on if your image op is hardware accelerated or not, check the source code of the underlying JDK class that
     * actually executes the Op code, <a href= "http://www.docjar.com/html/api/sun/awt/image/ImagingLib.java.html"
     * >sun.awt.image.ImagingLib</a>.
     *
     * <p>
     * <strong>TIP</strong>: This operation leaves the original <code>src</code>
     * image unmodified. If the caller is done with the <code>src</code> image after getting the result of this
     * operation, remember to call {@link BufferedImage#flush()} on the <code>src</code> to free up native resources and
     * make it easier for the GC to collect the unused image.
     *
     * @param src
     *         The image that will have the ops applied to it.
     * @param ops
     *         <code>1</code> or more ops to apply to the image.
     * @return a new {@link BufferedImage} that represents the <code>src</code> with all the given operations applied to
     *         it.
     ...
     *         if one of the given {@link BufferedImageOp}s fails to apply. These exceptions bubble up from the inside
     *         of most of the {@link BufferedImageOp} implementations and are explicitly defined on the imgscalr API to
     *         make it easier for callers to catch the exception (if they are passing along optional ops to be applied).
    ...
    /**
     * Used to crop the given <code>src</code> image from the top-left corner and applying any optional {@link
     * BufferedImageOp}s to the result before returning it.
     *
     * <p>
     * <strong>TIP</strong>: This operation leaves the original <code>src</code>
     * image unmodified. If the caller is done with the <code>src</code> image after getting the result of this
     ...
     *         unmodified.
     * @return a new {@link BufferedImage} representing the cropped region of the <code>src</code> image with any
     *         optional operations applied to it.
     * @throws IllegalArgumentException
     *         if <code>src</code> is <code>null</code>.
     * @throws IllegalArgumentException
     *         if any coordinates of the bounding crop box is invalid within the bounds of the <code>src</code> image
     *         (e.g. negative or too big).
     * @throws ImagingOpException
     *         if one of the given {@link BufferedImageOp}s fails to apply. These exceptions bubble up from the inside
     *         of most of the {@link BufferedImageOp} implementations and are explicitly defined on the imgscalr API to
     *         make it easier for callers to catch the exception (if they are passing along optional ops to be applied).
    ...
    /**
     * Used to crop the given <code>src</code> image and apply any optional {@link BufferedImageOp}s to it before
    ...
    /**
     * Used to apply padding around the edges of an image using {@link Color#BLACK} to fill the extra padded space and
     * then return the result.
     *
     * <p>
     * The amount of <code>padding</code> specified is applied to all sides; more specifically, a <code>padding</code>
     * of <code>2</code> would add 2 extra pixels of space (filled by the given <code>color</code>) on the top, bottom,
     * left and right sides of the resulting image causing the result to be 4 pixels wider and 4 pixels taller than the
     * <code>src</code> image.
     *
     * <p>
     * <strong>TIP</strong>: This operation leaves the original <code>src</code>
    ...
    /**
     * Used to apply padding around the edges of an image using the given color to fill the extra padded space and then
     * return the result. {@link Color}s using an alpha channel (i.e. transparency) are supported.
     *
     * <p>
     * The amount of <code>padding</code> specified is applied to all sides; more specifically, a <code>padding</code>
     * of <code>2</code> would add 2 extra pixels of space (filled by the given <code>color</code>) on the top, bottom,
     ...
     *         <code>src</code> is returned unmodified.
     * @param color
     *         The color to fill the padded space with. {@link Color}s using an alpha channel (i.e. transparency) are
     *         supported.
     * @param ops
     *         <code>0</code> or more ops to apply to the image. If
     *         <code>null</code> or empty then <code>src</code> is return
    ...
    /**
     * Resize a given image (maintaining its original proportion) to a width and height no bigger than
     * <code>targetSize</code> and apply the given {@link BufferedImageOp}s (if any) to the result before returning it.
     *
     * <p>
     ...
     *         sharpen, blur, etc.) that can be applied to the final result before returning the image.
    ...
    /**
     * Resize a given image (maintaining its original proportion) to a width and height no bigger than
     * <code>targetSize</code> using the given scaling method and apply the given {@link BufferedImageOp}s (if any) to
    ...
    /**
     * Resize a given image (maintaining its original proportion) to a width and height no bigger than
     * <code>targetSize</code> (or fitting the image to the given WIDTH or HEIGHT explicitly, depending on the {@link
     * Mode} specified) and apply the given {@link BufferedImageOp}s (if any) to the result before returning it.
     *
     * <p>
     ...
     *         image to the given width ({@link Mode#FIT_TO_WIDTH}) or fitting the image to the given height ({@link
     *         Mode#FIT_TO_HEIGHT}). If {@link Mode#AUTOMATIC} is passed in, imgscalr will calculate proportional
     *         dimensions for the scaled image based on its orientation (landscape, square or portrait). Unless you have
     *         very specific size requirements, most of the time you just want to use {@link Mode#AUTOMATIC} to "do the
     *         right thing".
     * @param targetSize
     *         The target width and height (square) that you wish the image to fit within.
     * @param ops
     ...
     * Resize a given image (maintaining its original proportion) to a width and height no bigger than
     * <code>targetSize</code> (or fitting the image to the given WIDTH or HEIGHT explicitly, depending on the {@link
     * Mode} specified) using the given scaling method and apply the given {@link BufferedImageOp}s (if any) to the
    ...
    /**
     * Resize a given image (maintaining its original proportion) to the target width and height and apply the given
    ...
    /**
     * Resize a given image (maintaining its original proportion) to the target width and height using the given scaling
     * method and apply the given {@link BufferedImageOp}s (if any) to the result before returning it.
     *
     * <p>
     * A mode of {@link Mode#AUTOMATIC} is used.
     *
     ...
     * to the given WIDTH or HEIGHT explicitly, depending on the {@link Mode} specified) and apply the given {@link
     ...
     * and apply the given {@link BufferedImageOp}s (if any) to the result before returning it.
     *
     * <p>
     * <strong>TIP</strong>: See the class description to understand how this
     * class handles recalculation of the <code>targetWidth</code> or
    ...
    /**
     * Used to apply a {@link Rotation} and then <code>0</code> or more {@link BufferedImageOp}s to a given image and
     * return the result.
     *
     * <p>
     * <strong>TIP</strong>: This operation leaves the original <code>src</code>
     ...
     * @param ops
     *         Zero or more optional image operations (e.g. sharpen, blur, etc.) that can be applied to the final result
    ...
    /**
     * Used to copy a {@link BufferedImage} from a non-optimal type into a new {@link BufferedImage} instance of an
     * optimal type (RGB or ARGB). If
     * <code>src</code> is already of an optimal type, then it is returned
     * unmodified.
     *
     * <p>
     * This method is meant to be used by any calling code (imgscalr's or otherwise) to convert any inbound image from a
     * poorly supported image type into the 2 most well-supported image types in Java2D ( {@link
     * BufferedImage#TYPE_INT_RGB} or {@link BufferedImage#TYPE_INT_ARGB} ) in order to ensure all subsequent graphics
     * operations are performed as efficiently and correctly as possible.
     *
     * <p>
     * When using Java2D to work with image types that are not well supported, the results can be anything from
     * exceptions bubbling up from the depths of Java2D to images being completely corrupted and just returned as solid
     * black.
     *
     * @param src
    ...
    /**
     * Used to create a {@link BufferedImage} with the given dimensions and the most optimal RGB TYPE ( {@link
     * BufferedImage#TYPE_INT_RGB} or {@link BufferedImage#TYPE_INT_ARGB} ) capable of being rendered into from the
     * given <code>src</code>.
     *
     * <p>
     * This does not perform a copy of the image data from <code>src</code> into the result image; see {@link
     * #copyToOptimalImage(BufferedImage)} for that.
     *
     * <p>
     * We force all rendering results into one of these two types, avoiding the case where a source image is of an
     * unsupported (or poorly supported) format by Java2D causing the rendering result to end up looking terrible
     ...
     * @return a new {@link BufferedImage} representing the most optimal target image type that <code>src</code> can be
     *         rendered into.
     * @throws IllegalArgumentException
     *         if <code>width</code> or <code>height</code> are &lt; 0.
     * @see <a href="http://www.mail-archive.com/java2d-interest@capra.eng.sun.com/msg05621.html">How Java2D handles
     *         poorly supported image types</a>
    ...
    /**
     * Used to create a {@link BufferedImage} with the most optimal RGB TYPE ( {@link BufferedImage#TYPE_INT_RGB} or
     * {@link BufferedImage#TYPE_INT_ARGB} ) capable of being rendered into from the given <code>src</code>. The width
     * and height of both images will be identical.
     *
     * <p>
     * This does not perform a copy of the image data from <code>src</code> into the result image; see {@link
     * #copyToOptimalImage(BufferedImage)} for that.
    ...
    /**
     * Used to determine the scaling {@link Method} that is best suited for scaling the image to the targeted
     * dimensions.
     *
     * <p>
     * This method is intended to be used to select a specific scaling {@link Method} when a {@link Method#AUTOMATIC}
     * method is specified. This method utilizes the {@link Scalr#THRESHOLD_QUALITY_BALANCED} and {@link
     * Scalr#THRESHOLD_BALANCED_SPEED} thresholds when selecting which method should be used by comparing the primary
     * dimension (width or height) against the threshold and seeing where the image falls. The primary dimension is
     * determined by looking at the orientation of the image: landscape or square images use their width and
     * portrait-oriented images use their height.
     *
     * @param targetWidth
     ...
     * @param ratio
     *         A height/width ratio used to determine the orientation of the image so the primary dimension (width or
     *         height) can be selected to test if it is greater than or less than a particular threshold.
     * @return the fastest {@link Method} suited for scaling the image to the specified dimensions while maintaining a
     *         good-looking result.
     */
     ...
     * into Objects when building out the <code>params</code> array, care should be taken not to call this method with
     * primitive values unless {@link Scalr#DEBUG} is <code>true</code>; otherwise the VM will be spending time
     * performing unnecessary auto-boxing calculations.
     *
     * @param depth
     *         The indentation level of the log message.
     * @param message
     *         The log message in <a href= "http://download.oracle.com/javase/6/docs/api/java/util/Formatter.html#syntax"
     ...
     *         The {@link RenderingHints} interpolation value used to indicate the method that {@link Graphics2D} should
     *         use when scaling the image.
     * @return the result of scaling the original <code>src</code> to the given dimensions using the given interpolation
     *         method.
     */
     ...
     * using BICUBIC interpolation as well as the explicit "flush()" operation on the interim BufferedImage instances to
     ...
     *         The target height for the scaled image.
     * @param scalingMethod
     *         The scaling method specified by the user (or calculated by imgscalr) to use for this incremental scaling
     *         operation.
     * @param interpolationHintValue
     *         The {@link RenderingHints} interpolation value used to indicate the method that {@link Graphics2D} should
     *         use when scaling the image.
    ...
    public enum Method {
        /**
         * Used to indicate that the scaling implementation should decide which method to use in order to get the best
         * looking scaled image in the least amount of time.
         *
         * <p>
         * The scaling algorithm will use the {@link Scalr#THRESHOLD_QUALITY_BALANCED} or {@link
         * Scalr#THRESHOLD_BALANCED_SPEED} thresholds as cut-offs to decide between selecting the <code>QUALITY</code>,
         * <code>BALANCED</code> or <code>SPEED</code> scaling algorithms.
         *
         * <p>
         * By default the thresholds chosen will give nearly the best looking result in the fastest amount of time. We
         * intend this method to work for 80% of people looking to scale an image quickly and get a good looking
         * result.
         */
        AUTOMATIC,
        /**
         * Used to indicate that the scaling implementation should scale as fast as possible and return a result. For
         * smaller images (800px in size) this can result in noticeable aliasing but it can be a few magnitudes times
         * faster than using the QUALITY method.
         */
        SPEED,
        /**
         * Used to indicate that the scaling implementation should use a scaling operation balanced between SPEED and
         * QUALITY. Sometimes SPEED looks too low quality to be useful (e.g. text can become unreadable when scaled
         * using SPEED) but using QUALITY mode will increase the processing time too much. This mode provides a "better
         * than SPEED" quality in a "less than QUALITY" amount of time.
         */
        BALANCED,
        /**
         * Used to indicate that the scaling implementation should do everything it can to create as nice of a result as
         * possible. This approach is most important for smaller pictures (800px or smaller) and less important for
         * larger pictures as the difference between this method and the SPEED method become less and less noticeable as
         * the source-image size increases. Using the AUTOMATIC method will automatically prefer the QUALITY method when
         * scaling an image down below 800px in size.
         */
        QUALITY,
        /**
         * Used to indicate that the scaling implementation should go above and beyond the work done by {@link
         * Method#QUALITY} to make the image look exceptionally good at the cost of more processing time. This is
         * especially evident when generating thumbnails of images that look jagged with some of the other {@link
    ...
    public enum Mode {
         ...
         *
         * <p>
         * It is recommended that you use one of the other <code>FIT_TO</code> modes or {@link Mode#AUTOMATIC} if you
         * want the image to look correct, but if dimension-fitting is the #1 priority regardless of how it makes the
         * image look, that is what this mode is for.
         */
        FIT_EXACT,
        /**
         * Used to indicate that the scaling implementation should calculate dimensions for the largest image that fit
         * within the bounding box, without cropping or distortion, retaining the original proportions.
         */
        BEST_FIT_BOTH,
        /**
    ...
    public enum Rotation {
        /**
         * 90-degree, clockwise rotation (to the right). This is equivalent to a quarter-turn of the image to the right;
         * moving the picture on to its right side.
         */
        CW_90,
        /**
         * 180-degree, clockwise rotation (to the right). This is equivalent to 1 half-turn of the image to the right;
         * rotating the picture around until it is upside down from the original position.
         */
        CW_180,
        /**
         * 270-degree, clockwise rotation (to the right). This is equivalent to a quarter-turn of the image to the left;
         * moving the picture on to its left side.
         */
        CW_270,
        /**
         ...
         *
         * <p>
         * This is not a standard rotation around a center point, but instead creates the mirrored reflection of the
         * image horizontally.
         *
         * <p>
         * More specifically, the vertical orientation of the image stays the same (the top stays on top, and the bottom
         ...
         *
         * <p>
         * More specifically, the horizontal orientation of the image stays the same (the left stays on the left and the
