...
import com.servercurio.fabric.image.scalr.Scalr.Method;
import com.servercurio.fabric.image.scalr.Scalr.Mode;
import com.servercurio.fabric.image.scalr.Scalr.Rotation;
import java.awt.*;
...
/**
 * Class used to provide the asynchronous versions of all the methods defined in {@link Scalr} for the purpose of
 * efficiently handling large amounts of image operations via a select number of processing threads asynchronously.
 * <p/>
 * Given that image-scaling operations, especially when working with large images, can be very hardware-intensive (both
 * CPU and memory), in large-scale deployments (e.g. a busy web application) it becomes increasingly important that the
 * scale operations performed by imgscalr be manageable so as not to fire off too many simultaneous operations that the
 * JVM's heap explodes and runs out of memory or pegs the CPU on the host machine, staving all other running processes.
 * <p/>
 * Up until now it was left to the caller to implement their own serialization or limiting logic to handle these
 * use-cases. Given imgscalr's popularity in web applications it was determined that this requirement be common enough
 * that it should be integrated directly into the imgscalr library for everyone to benefit from.
 * <p/>
 * Every method in this class wraps the matching methods in the {@link Scalr} class in new {@link Callable} instances
 * that are submitted to an internal {@link ExecutorService} for execution at a later date. A {@link Future} is returned
 * to the caller representing the task that is either currently performing the scale operation or will at a future date
 * depending on where it is in the {@link ExecutorService}'s queue. {@link Future#get()} or {@link Future#get(long,
 * TimeUnit)} can be used to block on the
...
public class AsyncScalr {
    /**
     * System property name used to set the number of threads the default underlying {@link ExecutorService} will use to
     * process async image operations.
     * <p/>
    ...
    /**
     * Number of threads the internal {@link ExecutorService} will use to simultaneously execute scale requests.
     * <p/>
    ...
    /**
     * Used to get access to the internal {@link ExecutorService} used by this class to process scale operations.
     * <p/>
     * <strong>NOTE</strong>: You will need to explicitly shutdown any service
     * currently set on this class before the host JVM exits.
     * <p/>
     * You can call {@link ExecutorService#shutdown()} to wait for all scaling operations to complete first or call
     * {@link ExecutorService#shutdownNow()} to kill any in-process operations and purge all pending operations before
     * exiting.
     * <p/>
     * Additionally you can use {@link ExecutorService#awaitTermination(long, TimeUnit)} after issuing a shutdown
     * command to try and wait until the service has finished all tasks.
     *
     * @return the current {@link ExecutorService} used by this class to process scale operations.
     */
    ...
    /**
     * Used to verify that the underlying <code>service</code> points at an active {@link ExecutorService} instance that
     * can be used by this class.
     * <p/>
     * If <code>service</code> is <code>null</code>, has been shutdown or terminated then this method will replace it
     * with a new {@link ExecutorService} by calling the {@link #createService()} method and assigning the returned
     * value to <code>service</code>.
     * <p/>
    ...
    /**
     * Default {@link ThreadFactory} used by the internal {@link ExecutorService} to creates execution {@link Thread}s
     * for image scaling.
     * <p/>
     * More or less a copy of the hidden class backing the {@link Executors#defaultThreadFactory()} method, but exposed
     * here to make it easier for implementors to extend and customize.
     *
     * @author Doug Lea
     * @author Riyad Kalla (software@thebuzzmedia.com)
     * @since 4.0
    ...
    protected static class DefaultThreadFactory implements ThreadFactory {
        ...
        /**
         * Used to create a {@link Thread} capable of executing the given {@link Runnable}.
         * <p/>
         * Thread created by this factory are utilized by the parent {@link ExecutorService} when processing queued up
         * scale operations.
         */
    ...
    /**
     * An extension of the {@link DefaultThreadFactory} class that makes two changes to the execution {@link Thread}s it
     * generations:
     * <ol>
    ...
    protected static class ServerThreadFactory extends DefaultThreadFactory {
        /**
         * Overridden to set <code>daemon</code> property to <code>true</code> and decrease the priority of the new
         * thread to {@link Thread#MIN_PRIORITY} before returning it.
         */
        @Override
        public Thread newThread(Runnable r) {
